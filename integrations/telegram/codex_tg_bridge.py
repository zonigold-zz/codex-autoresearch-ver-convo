    ...
    def _loop(self) -> None:
        while True:
            ...
            with self._lock:
                stop = self._stop
                dirty = self._dirty
                active = self.status in {"starting", "running", "thinking"}
            if dirty or active:
...
class CodexRunner:
    ...
    def _filtered_passthrough_args(self, *, session_id: str | None = None) -> list[str]:
        """Drop bridge-managed flags and resume-incompatible Codex flags."""
        banned = {"--json", "--experimental-json"}
        resume_flags_with_value = {"--add-dir", "--ask-for-approval", "--sandbox"}

        filtered: list[str] = []
        i = 0
        while i < len(self.passthrough_args):
            arg = self.passthrough_args[i]

            if arg in banned:
                i += 1
                continue

            if session_id is not None and arg in resume_flags_with_value:
                i += 2
                continue
    ...
    def run_turn(
        ...
        *,
        ...
        command += self._filtered_passthrough_args(session_id=session_id)
...
HELP_TEXT = """Commands
/prompt_begin
          Start collecting a long prompt across multiple Telegram messages.
/prompt_end
          Send the collected long prompt to Codex.
/prompt_cancel
          Discard the collected long prompt.
...
/autoresearch [TEXT]
          Wrap the message as a `$codex-autoresearch` prompt before sending it.
AUTONOMOUS_LONG_RUN
          Include this token in a prompt to keep Codex auto-continuing across turns.
...
# -----------------------------------------------------------------------------
def build_autoresearch_prompt(text: str) -> str:
    body = text.strip()
    if not body:
        return "$codex-autoresearch"
    return f"$codex-autoresearch\n{body}"



def is_autonomous_hard_blocker_message(text: str | None) -> bool:
    """Return True when Codex reports a blocker that should stop auto-continuation."""
    if not text:
        return False
...
def build_autonomous_continue_prompt(previous_message: str | None, turn_index: int, max_turns: int) -> str:
    """Build the next continuation prompt for long autonomous research sessions."""
    summary = (previous_message or "(No final message captured.)").strip()
    if len(summary) > 2500:
...
Continue autonomously from the previous turn.
Do not reset the session.
Do not re-plan from scratch.
Do not stop for assessment-only milestones, anchor registration, summary-only updates, or next-phase recommendation.

Autonomous continuation turn: {turn_index + 1}/{max_turns}

Previous turn summary:
{summary}

Use the current working tree, committed registry manifests, and approved E-drive artifacts as the only sources of truth.
Preserve uncommitted changes from the current launched session.

Read the current evidence state, identify the weakest unresolved scientific blocker, implement the minimum safe fix, run smoke, run real foreground execution, compare against accepted anchors, update summaries/manifests, and continue.

A phase counts as substantive only if it has implementation or justified reuse, smoke pass, real run or valid approved artifact reuse, comparison artifacts, updated researcher-facing manifest/summary, real metrics or audit outcomes, and a decision-level conclusion.

If results are zero-delta or conflicting, enter diagnostic mode: audit evaluator sensitivity, preprocessing dominance, label/event validity, code-path distinctness, and dataset-specific failure; patch minimum safe code; rerun smoke and affected real checks.

Stop only for a true hard blocker, verify/guard failure, raw write attempt, output path violation, subject/session leakage, or completion of the autonomous budget.

Report only when the stop rule is met.
"""
...
def main() -> int:
    ...
    try:
        ...
        prompt_buffer_active = False
        prompt_buffer_parts: list[str] = []
        ...
        def run_in_thread(prompt: str) -> None:
            ...
            try:
                ...
                autonomous_mode = (
                    "AUTONOMOUS_LONG_RUN" in prompt
                    or os.environ.get("CODEX_TG_AUTO_CONTINUE", "").strip() == "1"
                )
                max_turns = _get_env_int("CODEX_TG_AUTONOMOUS_TURNS", 8, min_value=1)
                max_hours = _get_env_float("CODEX_TG_AUTONOMOUS_HOURS", 3.0, min_value=0.1)
                ...
                while True:
                    log(f"YOU: {current_prompt}")
                    if autonomous_mode:
                        log(f"[auto] starting turn {completed_turns + 1}/{max_turns}")
                    ...
                    if is_autonomous_hard_blocker_message(final_message):
                        log("[auto] hard blocker detected; stopping autonomous continuation")
                        break

                    if completed_turns >= max_turns:
                        log("[auto] max autonomous turns reached")
                        break

                    if time.time() >= deadline:
                        log("[auto] max autonomous time reached")
                        break

                    current_prompt = build_autonomous_continue_prompt(
                        final_message,
                        completed_turns,
                        max_turns,
                    )
                ...
                if final_message:
                    if autonomous_mode:
                        outbox.send(
                            redact_secrets(
                                f"Autonomous session completed after {completed_turns} turn(s).\n\n{final_message}"
                            )
                        )
                    else:
        ...
        try:
            while True:
                ...
                for update in updates:
                    ...
                    # Long prompt collection mode.
                    # Telegram text messages are limited, so this lets the operator
                    # send a long Codex prompt in several chunks and then dispatch it
                    # as one prompt with /prompt_end.
                    command_text = text.split()[0] if text.split() else text
                    if prompt_buffer_active:
                        if command_text in {"/prompt_end", f"/prompt_end@{bot_username}"}:
                            text = "\n".join(prompt_buffer_parts).strip()
                            prompt_buffer_active = False
                            prompt_buffer_parts = []
                            if not text:
                                outbox.send("Prompt buffer is empty. Nothing was sent.")
                                continue
                            # Fall through to normal Codex dispatch below.
                        elif command_text in {"/prompt_cancel", f"/prompt_cancel@{bot_username}"}:
                            prompt_buffer_active = False
                            prompt_buffer_parts = []
                            outbox.send("Prompt buffer discarded.")
                            continue
                        else:
                            prompt_buffer_parts.append(text)
                            total_chars = sum(len(part) for part in prompt_buffer_parts) + max(0, len(prompt_buffer_parts) - 1)
                            outbox.send(
                                f"Prompt chunk stored: {len(prompt_buffer_parts)} chunk(s), {total_chars} chars. "
                                "Send /prompt_end to dispatch or /prompt_cancel to discard."
                            )
                            continue
                    elif command_text in {"/prompt_begin", f"/prompt_begin@{bot_username}"}:
                        if turn_guard.locked() or runner.is_running():
                            outbox.send("Codex is currently running. Use /cancel first, then try again.")
                            continue
                        prompt_buffer_active = True
                        prompt_buffer_parts = []
                        outbox.send(
                            "Prompt buffer started. Send the long prompt in multiple messages, "
                            "then send /prompt_end. Use /prompt_cancel to discard."
                        )
                        continue
                    elif command_text in {"/prompt_end", f"/prompt_end@{bot_username}"}:
                        outbox.send("No active prompt buffer. Send /prompt_begin first.")
                        continue
                    elif command_text in {"/prompt_cancel", f"/prompt_cancel@{bot_username}"}:
                        outbox.send("No active prompt buffer.")
                        continue
                    ...
                    if text.startswith("/autoresearch"):
                        ...
                        if not payload:
                            outbox.send(
                                ...
                                "Example\n"
                                "/autoresearch Mode: loop\\nGoal: strengthen the manuscript-opening evidence chain"
                            )
                            continue
                        text = build_autoresearch_prompt(payload)
