...
Foreground 和 background 共享完全相同的实验协议。差别只在于循环运行的位置：foreground 在当前 Codex 会话内执行，background 则交给 detached runtime controller。两者都会持续运行到被中断（无界）或恰好运行 N 次迭代（通过 `Iterations: N` 设定上限）。
